...
export const GuessMap: React.FC<GuessMapProps> = ({ className = '', isKeyboardExpanded = false }) => {
  ...
  useEffect(() => {
    if (gameStatus === 'PLAYING') {
      ...
      if (mapInstanceRef.current && telemetry.apiMode !== 'REAL') {
        const defaultCenter: [number, number] = [20, 0];
        const defaultZoom = 2;
        mapInstanceRef.current.setView(defaultCenter, defaultZoom, { animate: false });
      }
    }
  }, [currentRoundIndex, gameStatus, telemetry.apiMode]);
  ...
  return (
    <div
      ...
      className={`relative transition-all duration-300 ease-in-out z-30 shadow-2xl rounded-3xl overflow-hidden border border-slate-200 bg-white/95 backdrop-blur-2xl flex flex-col max-w-[calc(100vw-2rem)] max-h-[calc(100vh-5rem)] ${
        isFullyExpanded
        ...
        <>
          ...
          {gameStatus === 'PLAYING' && (
            <div className="px-3 py-2 bg-white/95 border-t border-slate-200 flex items-center justify-between gap-2.5 z-20">
              ...
              {/* Middle: Map / Satellite Hybrid Mode Pill */}
              <div className="flex items-center bg-slate-100 p-0.5 rounded-lg border border-slate-200 shrink-0">
                <span className="px-2.5 py-1 text-[11px] font-bold rounded-md bg-white text-slate-900 shadow-xs border border-slate-200/80">
                  Map
                </span>
              </div>
              ...
              <button
                ...
                className={`min-w-[140px] sm:min-w-[160px] px-8 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider shadow-md flex items-center justify-center gap-2 transition-all shrink-0 ${
                  selectedGuess
                    ? 'bg-emerald-600 hover:bg-emerald-700 text-white hover:scale-105 active:scale-95 cursor-pointer shadow-emerald-600/20'
